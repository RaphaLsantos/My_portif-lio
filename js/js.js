...
function initCursor() {
  ...
  if (window.innerWidth > 768) {
    ...
    const interactiveElements = document.querySelectorAll(
      "a, button, .social-link, .skill-item, .project-card, .contact-item",
    );
    interactiveElements.forEach((el) => {
      ...
      el.addEventListener("mouseleave", () => {
        ...
        cursor.style.borderColor = "var(--accent)";fprojects
...
function initScrollReveal() {
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry, index) => {
      if (entry.isIntersecting) {
        ...
        entry.target.style.animation = entry.target.classList.contains(
          "reveal-scale",
        )
  ...
  document
    .querySelectorAll(
      ".reveal-text, .reveal-scale, .project-card, .skill-category, .certificate-card, .contact-item",
    )
    .forEach((el) => {
      observer.observe(el);
    });
...
function initScrollAnimations() {
  ...
  document
    .querySelectorAll(
      ".project-card, .skill-category, .certificate-card, .contact-item",
    )
    .forEach((el) => {
      el.style.opacity = "0";
      observer.observe(el);
...
function initCardHoverEffect() {
  const cards = document.querySelectorAll(
    ".project-card, .skill-category, .certificate-card, .about-card",
  );
...
window.addEventListener("scroll", () => {
  ...
  if (window.pageYOffset > 50) {
    sidebar.style.background =
      "linear-gradient(180deg, rgba(17, 22, 51, 0.98) 0%, rgba(10, 14, 39, 0.95) 100%)";
  } else {
    sidebar.style.background =
      "linear-gradient(180deg, rgba(17, 22, 51, 0.95) 0%, rgba(10, 14, 39, 0.95) 100%)";
  }
});
